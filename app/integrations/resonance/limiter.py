...
Both live in SQLite rather than in process memory. start.sh defaults to one
worker, but PKTLOG_WORKERS can raise it and app/main.py's direct-run path uses
two — an in-process counter silently multiplies its own limit once there is
...
log = logging.getLogger("pktlog.resonance.limiter")
