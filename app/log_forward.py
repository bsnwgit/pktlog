...
pktlog's own log records are written to the local app_logs table for the
...
log = logging.getLogger("pktlog.log_forward")
...
class SyslogForwardHandler(logging.Handler):
    ...
    def __init__(
        ...
        self._thread = threading.Thread(
            target=self._run, name="pktlog-log-forward", daemon=True
        )
        self._thread.start()
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if record.name.startswith("pktlog.log_forward"):
            return
        try:
...
def send_test_message(host: str, port: int, protocol: str, app_name: str = "pktlog") -> dict:
    ...
    try:
        rec = logging.LogRecord(
            name="pktlog.test", level=logging.INFO, pathname=__file__, lineno=0,
            msg="pktlog log forwarding test message", args=(), exc_info=None,
        )
        h.emit(rec)
