...
credentials:'include'. pktlog's access token lives in memory by design, so there
...
log = logging.getLogger("pktlog.api.resonance")
...
async def _embed_config(db: aiosqlite.Connection) -> dict:
    ...
    return {
        ...
        "exclude_paths": await _get(db, "resonance_exclude_paths", []) or [],
    }
