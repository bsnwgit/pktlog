...
- **Syslog Explorer's row-expand dropdown doesn't show every field.** Clicking a log row expands `ExpandedRow` (frontend/src/pages/SyslogExplorer.tsx), which currently shows `received_at`, `collector_name`/`collector_ip`, `source_name`/`source_ip`, `facility_name`/`facility`, `org`, `log_group`, `site`, `message`, and conditionally `pid` (only if non-empty) and `raw` (only if non-empty and different from `message`) — but `timestamp` and `severity`/`severity_name` aren't in the dropdown at all (only shown in the collapsed row), so the expanded view isn't a complete, unconditional listing of all 17 `syslog_events` columns. User wants every field always present as its own row/field in the dropdown, not split between the collapsed row and an inconsistent subset in the expansion, and not conditionally hidden when empty.
