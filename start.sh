...
# Auto-detects /mnt/software/pktlog/ssl/server.crt + server.key on startup.
# To enable HTTPS: upload cert/key via Settings → Integrations → SSL / TLS, then restart.
# To disable HTTPS: remove the cert via Settings (or rm /mnt/software/pktlog/ssl/server.*), then restart.

SSL_CERT="/mnt/software/pktlog/ssl/server.crt"
SSL_KEY="/mnt/software/pktlog/ssl/server.key"
UVICORN="/mnt/software/pktlog/venv/bin/uvicorn"
