...
function LogForwardTester({ host, port, protocol }: { host: string; port: number; protocol: string }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      setOk(r.ok)
      setResult(r.ok
        ? `Sent 1 message to ${r.target} — check pktLog for "pktLog log forwarding test message"`
...
export default function Settings() {
  ...
  const ALL_BUNDLE_FILES = ['pktlog.db', 'config.yaml', 'flows.csv.gz']
            ...
            {securityTab === 'ai' && (
              <Section title="AI Assistant" onSave={aiAssistantSave.save} saving={aiAssistantSave.saving} saved={aiAssistantSave.saved} error={aiAssistantSave.error}
                help={{
                  ...
                  content: <>
                    <p>Providers are tried top to bottom — <span className="text-gray-300 font-medium">local / self-hosted providers first</span>, since they keep your log data on hardware you control, then cloud providers as a fallback. Turn on as many as you like; the first enabled provider with valid config answers each question in the chat panel.</p>
              ...
              >
                ...
                >
                  ...
                  <MiniField label="Model" hint="Haiku is fast and cost-effective for syslog-context questions">
                    <SelectInput
      ...
      {dataTab === 'logforward' && (
        <Section title="Log Forwarding" onSave={logForwardSave.save} saving={logForwardSave.saving} saved={logForwardSave.saved} error={logForwardSave.error}
          help={{
            ...
            content: <>
              <p>Ships pktLog's own application log to a syslog collector — normally <span className="text-gray-300 font-medium">pktLog</span>, which listens on port <code className="text-gray-400">5514</code> — so this app's logs sit alongside the rest of the estate instead of only in its local Logs page.</p>
              ...
              <p>Delivery is fire-and-forget on a background thread — if the collector is unreachable, lines are dropped and counted rather than blocking or crashing pktLog. Use <span className="text-gray-300 font-medium">Send test message</span> to confirm the path end to end.</p>
        ...
        >
          ...
          <Field label="Application name" hint="Appears as the APP-NAME field in the syslog message">
            <TextInput value={str('log_forward_app_name') || 'pktlog'} onChange={v => set('log_forward_app_name', v)} placeholder="pktlog" />
          </Field>
          <LogForwardTester
          ...
          <div className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
            ...
            <div className="px-6 py-4">
              <p className="text-xs text-gray-400 mb-3">
                {systemInfo?.app_name ?? 'pktLog'} is built with the following open-source software:
              </p>
