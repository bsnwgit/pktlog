-- pktLog migration 014 — resonance embed integration
--
-- Three pieces of state the embed integration needs to survive a restart, all
-- deliberately in SQLite rather than in-process: start.sh defaults to one
-- worker but PKTLOG_WORKERS can raise it, and an in-process counter silently
-- multiplies its own limit once there is more than one.
--
...
--                       geometric per-IP backoff (to 300s); pktlog is a single
--                       IP, so continuing to call with a bad key would take the
--                       widget down for every user at once. The breaker stops
--                       calling instead, and the Settings panel reads this row
--                       so a paused integration says so rather than looking
