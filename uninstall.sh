#!/bin/bash
# pktLog uninstall script — Ubuntu Server 22.04/24.04 LTS
...
# credential encryption key), pktlog.db and its -wal/-shm, logs, backups and
...
# WorkingDirectory, then $PKTLOG_INSTALL_DIR, then /opt/pktlog.

set -euo pipefail

APP="pktlog"
DISPLAY="pktLog"
UNIT_PATH="/etc/systemd/system/pktlog.service"
...
# migration — plain *.db does not match "pktlog.db.pre-upgrade".
...
if [ "$PURGE" -eq 1 ]; then
    for entry in logs backups ssl data ingest_journal; do
    ...
    for entry in config.yaml pktlog.db logs backups ssl pktlog_data.duckdb ingest_journal; do
        if [ -e "$INSTALL_DIR/$entry" ]; then
            echo "    $entry"
        fi
    done
...
if [ "$PURGE" -eq 1 ] && command -v clickhouse-client &>/dev/null; then
    if clickhouse-client --query "EXISTS DATABASE pktlog" 2>/dev/null | grep -q '^1$'; then
        echo ""
        echo "  ClickHouse database 'pktlog' still exists and was NOT dropped."
        echo "  To remove it as well:"
        echo "      clickhouse-client --query \"DROP DATABASE pktlog\""
    fi
fi
