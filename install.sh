...
PREV_INSTALL=0
REMOVE_EXISTING=0
UNIT_FILE="/etc/systemd/system/pktlog.service"
...
if [ "$PREV_INSTALL" -eq 1 ]; then
    ...
    echo "Found an existing pktLog installation at $INSTALL_DIR (version $PREV_VERSION)."
    ...
    elif [ -n "${PKTLOG_REMOVE_EXISTING:-}" ]; then
        REMOVE_EXISTING="$PKTLOG_REMOVE_EXISTING"
    elif [ -t 0 ]; then
...
DB_EXISTED=0
[ -f "$INSTALL_DIR/pktlog.db" ] && DB_EXISTED=1
